<script lang="ts">
	...
	async function gass() {
		...
		if (res.status === 201) {
			...
			error = 'Username sudah terdaftar!'; // Set pesan kesalahan jika username sudah ada
			setTimeout(() => {
    ...
    <ContainerSo>
        ...
        <BoxSo height="600px">
            <div class="box-content">
                <p>Let me<br>know<br>about you!</p>
                <hr />
				<label class="input-label">Fullname</label>
                <input type="text" id="fullname" class="input-field" placeholder="Fullname" bind:value={body.name} />
				...
				<button class="sign-up-button" on:click={gass}>Sign Up</button>
				<hr class="secondary-hr" />
				<a href="/signin" class="create-account-link">Sign In</a>
...
<style>
    ...
    .box-content .input-label {
        ...
        display: block; /* Tambahkan padding bawah untuk jarak kecil */
    }
    .box-content .input-field {
        ...
        font-size: 16px; /* Tambahkan margin bawah untuk jarak antara fields */
		margin-top: 5px;
    }

	.box-content .sign-up-button {
        width: 129px;
        height: 44px;
        border-radius: 10px;
    ...
    .box-content .sign-up-button:hover {
        background-color: #e0e0e0; /* Warna tombol saat di-hover */
    }

    .box-content .sign-up-button:active {
	...
	.popup {
        ...
        background-color: rgba(235, 79, 79, 0.9);
        color: white;
        padding: 15px;
        border-radius: 5px;
